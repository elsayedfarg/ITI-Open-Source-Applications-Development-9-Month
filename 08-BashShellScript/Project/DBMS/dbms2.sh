...
for (( i=1; i<=nocols; i++ )); do
        ...
        echo ""
        echo -n "Enter column $i $col_name datatype (int/string)?: "
        read col_type
...
insert_into_table() {
    ...
    for i in "${!cols[@]}"; do
        ...
        while true; do
            ...
            if [[ "$type" == "int" && ! "$data" =~ ^[0-9]+$ ]]; then
   # int bas 3shan string no need to check all good
...
# drop table  => can not read tables
# and all tables have the same problem because you have used this line
# ls "Databases/$currentdb/$tablename" | grep -v '\.meta$'
# this is the correct line ls "Databases/$currentdb/" | grep -v '\.meta$'

# drop table  => table dropped when you choose (n) need space between ! and check variable


# edits
# connect to database without entering database name (Done)
# list tables before drop (Done)
# list tables before insert (Done)
# list tables before the query (Done)
