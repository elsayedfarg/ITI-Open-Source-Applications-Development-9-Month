...
class BankAccount
  ...
  def deposit(amount)
    if amount > 0
      # BUG [3]: [logical error(deposit must add to the balance)] → FIX: [instead of -= use +=]
      @balance += amount
  ...
  def withdraw(amount)
    # BUG [5]: [logical error(validate balance before withdraw)] → FIX: [add guard]
    ...
    # BUG [1]: [syntax error(missing end)] → FIX: [add (end)]
  end

  def apply_interest
    # BUG [4]: [logical error(the old code overwrite the balance)] → FIX: [instead of = use +=]
  ...
  def display_info
    puts "Owner  : #{@owner}"
    # BUG [2]: [syntax error(interpolation problem)] → FIX: [add { instead of ( ]
